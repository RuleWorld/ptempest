...
calc_expressions ( N_Vector expressions, double * parameters )
{
    ...
    NV_Ith_S(expressions,5) = pow(10.0,NV_Ith_S(expressions,2));
    NV_Ith_S(expressions,6) = pow(10.0,NV_Ith_S(expressions,3));
    NV_Ith_S(expressions,7) = pow(10.0,NV_Ith_S(expressions,4));
...
{  
    NV_Ith_S(ratelaws,0) = NV_Ith_S(expressions,5)*NV_Ith_S(species,1)*NV_Ith_S(species,0);
    NV_Ith_S(ratelaws,1) = NV_Ith_S(expressions,6)*NV_Ith_S(species,2);
    NV_Ith_S(ratelaws,2) = NV_Ith_S(expressions,7)*NV_Ith_S(species,2);
...
calc_species_deriv ( realtype time, N_Vector species, N_Vector Dspecies, void * f_data )
{
    ...
    NV_Ith_S(Dspecies,1) = -NV_Ith_S(ratelaws,0) +NV_Ith_S(ratelaws,2) +NV_Ith_S(ratelaws,1);
    NV_Ith_S(Dspecies,2) = NV_Ith_S(ratelaws,0) -NV_Ith_S(ratelaws,2) -NV_Ith_S(ratelaws,1);
...
void mexFunction( int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] )
{
    ...
    {  mexErrMsgTxt("PARAMS must be a column vector with 5 elements.");  }
