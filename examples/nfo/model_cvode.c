...
**   parameters      : row vector of 14 parameters.
...
/* Problem Dimensions */
#define __N_PARAMETERS__   14
#define __N_EXPRESSIONS__  17
#define __N_OBSERVABLES__  5
#define __N_RATELAWS__     7
#define __N_SPECIES__      5
...
double rateLaw4 ( N_Vector expressions, N_Vector observables );
double rateLaw5 ( N_Vector expressions, N_Vector observables );
double rateLaw6 ( N_Vector expressions, N_Vector observables );
double rateLaw7 ( N_Vector expressions, N_Vector observables );


/* user-defined function definitions  */
/* user-defined function rateLaw4 */
double rateLaw4 ( N_Vector expressions, N_Vector observables )
{
    return (exp(NV_Ith_S(expressions,6))/(exp(NV_Ith_S(expressions,7))+(NV_Ith_S(observables,2)-NV_Ith_S(observables,1))));
}

/* user-defined function rateLaw5 */
double rateLaw5 ( N_Vector expressions, N_Vector observables )
{
    return (exp(NV_Ith_S(expressions,8))/(exp(NV_Ith_S(expressions,9))+NV_Ith_S(observables,1)));
}

/* user-defined function rateLaw6 */
double rateLaw6 ( N_Vector expressions, N_Vector observables )
{
    return (exp(NV_Ith_S(expressions,10))/(exp(NV_Ith_S(expressions,11))+(NV_Ith_S(observables,4)-NV_Ith_S(observables,3))));
}

/* user-defined function rateLaw7 */
double rateLaw7 ( N_Vector expressions, N_Vector observables )
{
    return (exp(NV_Ith_S(expressions,12))/(exp(NV_Ith_S(expressions,13))+NV_Ith_S(observables,3)));
}
...
calc_expressions ( N_Vector expressions, double * parameters )
{
    ...
    NV_Ith_S(expressions,14) = (exp(NV_Ith_S(expressions,3))*NV_Ith_S(expressions,0));
    NV_Ith_S(expressions,15) = exp(NV_Ith_S(expressions,4));
    NV_Ith_S(expressions,16) = exp(NV_Ith_S(expressions,5));
...
calc_observables ( N_Vector observables, N_Vector species, N_Vector expressions )
{
    ...
    NV_Ith_S(observables,1) = NV_Ith_S(species,2);
    NV_Ith_S(observables,2) = NV_Ith_S(species,1) +NV_Ith_S(species,2);
    NV_Ith_S(observables,3) = NV_Ith_S(species,4);
    NV_Ith_S(observables,4) = NV_Ith_S(species,3) +NV_Ith_S(species,4);
...
{  
    NV_Ith_S(ratelaws,0) = NV_Ith_S(expressions,14);
    NV_Ith_S(ratelaws,1) = NV_Ith_S(expressions,15)*NV_Ith_S(species,0);
    NV_Ith_S(ratelaws,2) = NV_Ith_S(expressions,16)*NV_Ith_S(species,0)*NV_Ith_S(species,4);
    NV_Ith_S(ratelaws,3) = rateLaw4(expressions,observables)*NV_Ith_S(species,1)*NV_Ith_S(species,0);
    NV_Ith_S(ratelaws,4) = rateLaw5(expressions,observables)*NV_Ith_S(species,2);
    NV_Ith_S(ratelaws,5) = rateLaw6(expressions,observables)*NV_Ith_S(species,3)*NV_Ith_S(species,2);
    NV_Ith_S(ratelaws,6) = rateLaw7(expressions,observables)*NV_Ith_S(species,4);
...
calc_species_deriv ( realtype time, N_Vector species, N_Vector Dspecies, void * f_data )
{
    ...
    /* calculate derivates */
    NV_Ith_S(Dspecies,0) = NV_Ith_S(ratelaws,0) -NV_Ith_S(ratelaws,2) -NV_Ith_S(ratelaws,1);
    NV_Ith_S(Dspecies,1) = -NV_Ith_S(ratelaws,3) +NV_Ith_S(ratelaws,4);
    NV_Ith_S(Dspecies,2) = NV_Ith_S(ratelaws,3) -NV_Ith_S(ratelaws,4);
    NV_Ith_S(Dspecies,3) = -NV_Ith_S(ratelaws,5) +NV_Ith_S(ratelaws,6);
    NV_Ith_S(Dspecies,4) = NV_Ith_S(ratelaws,5) -NV_Ith_S(ratelaws,6);
...
void mexFunction( int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] )
{
    ...
    {  mexErrMsgTxt("SPECIES_INIT must be a row vector with 5 elements.");  } 

    /* make sure params has correct dimensions */
    if ( (mxGetM(prhs[2]) != 1)  ||  (mxGetN(prhs[2]) != __N_PARAMETERS__) )
    {  mexErrMsgTxt("PARAMS must be a column vector with 14 elements.");  }
